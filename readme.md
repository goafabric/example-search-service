...
docker pull goafabric/example-search-service${ARCH}:3.0.0-SNAPSHOT && \
docker run --name example-search-service --rm -p50800:50800 -e "elasticsearch.url=172.17.0.1:9200" goafabric/example-search-service${ARCH}:3.0.0-SNAPSHOT

#docker run native
docker pull goafabric/example-search-service-native:3.0.0-SNAPSHOT && \
docker run --name example-search-service --rm -p50800:50800 -e "elasticsearch.url=172.17.0.1:9200" goafabric/example-search-service-native:3.0.0-SNAPSHOT -Xmx64m
